...
[[nodiscard]] bool run_smoke(const std::filesystem::path& runtime,
    ...
    if (runtime_pid == 0) {
        ...
        ::execl(runtime.c_str(), runtime.c_str(), "--trace=runtime,gui,process",
                target.c_str(), static_cast<char*>(nullptr));
        ::_exit(127);
    }
    if (trace_fd >= 0) {
        ::close(trace_fd);
    ...
    Window about = 0;
    if (display != nullptr) {
        for (int attempt = 0; attempt < 100 && about == 0; ++attempt) {
    ...
    bool passed = display != nullptr;
    if (passed && about != 0) {
        passed = send_close(display, about);
    }
    Window configuration = 0;
    for (int attempt = 0; passed && attempt < 30; ++attempt) {
        std::this_thread::sleep_for(100ms);
    ...
    if (display != nullptr) {
        if (configuration != 0) {
            ...
            passed = send_close(display, configuration);
        }
    ...
    if (!runtime_exited) {
        if (::getpgid(runtime_pid) == runtime_pid) {
            ::kill(-runtime_pid, SIGKILL);
        }
        (void)::waitpid(runtime_pid, nullptr, 0);
    }
    passed = passed && configuration != 0 && runtime_exited && WIFEXITED(runtime_status) &&
    ...
    passed = passed && trace.find("CreateDialogParamA") != std::string::npos &&
             trace.find("caption=\"About PuTTY\"") != std::string::npos &&
             trace.find("caption=\"PuTTY Configuration\"") != std::string::npos;
    if (!passed) {
        std::cerr << "smoke do PuTTY não confirmou About/configuração e saída limpa\n";
        std::cerr << trace;
    }
    std::filesystem::remove_all(staging, error);
    return passed;
...
int main(const int argc, char** const argv) {
    if (argc != 3) {
        std::cerr << "uso: putty_smoke <runtime> <putty.exe>\n";
        return 2;
    }
