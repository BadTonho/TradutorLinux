...
string(FIND "${report_output}" "execution-result: not-attempted" execution_result_position)
string(FIND "${report_output}" "compatibility:" compatibility_position)
if(execution_position EQUAL -1)
    message(FATAL_ERROR
        "Expected execution: not-attempted in the report:\n${report_output}")
endif()
if(execution_result_position EQUAL -1)
    message(FATAL_ERROR
        "Expected execution-result: not-attempted in the report:\n${report_output}")
endif()
if(compatibility_position EQUAL -1)
    message(FATAL_ERROR
        "Expected compatibility: line in the report:\n${report_output}")
endif()
...
foreach(dll_index RANGE 0 ${last_dll_index})
    ...
    string(FIND "${report_output}" "dll: ${dll_name}" dll_group_position)
    if(dll_group_position EQUAL -1)
        message(FATAL_ERROR
            "Report does not group imports under dll: ${dll_name}:\n${report_output}")
    endif()
    ...
    foreach(symbol_index RANGE 0 ${last_symbol_index})
        ...
        string(FIND "${report_output}" "import: ${symbol_name}" symbol_position)
        if(symbol_position EQUAL -1)
            message(FATAL_ERROR
                "Report does not list import: ${symbol_name} under ${dll_name}:\n${report_output}")
        endif()
    endforeach()
endforeach()
