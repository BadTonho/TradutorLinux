...
static const char kClassName[] = "tlkey";
static const char kWindowName[] = "Ola do Windows no Linux!";

static int g_q_shift = 0;      // WM_CHAR 'Q' (Shift+q)
static int g_ret = 0;          // WM_KEYDOWN VK_RETURN
static int g_left_up = 0;      // WM_KEYUP VK_LEFT

static void maybe_finish(hwnd_t hwnd) {
    if (g_q_shift != 0 && g_ret != 0 && g_left_up != 0) {
        DestroyWindow(hwnd);
    }
}
...
static lresult_t wndproc(hwnd_t hwnd, uint_t message, wparam_t wparam, lparam_t lparam) {
    ...
    if (message == 0x0102U) { /* WM_CHAR */
        if (wparam == (wparam_t)0x51U) { /* 'Q' (Shift+q) */
            g_q_shift = 1;
        }
        maybe_finish(hwnd);
        return 0;
    }
    if (message == 0x0100U) { /* WM_KEYDOWN */
        if (wparam == (wparam_t)0x0DU) { /* VK_RETURN */
            g_ret = 1;
        }
        maybe_finish(hwnd);
        return 0;
    }
    if (message == 0x0101U) { /* WM_KEYUP */
        if (wparam == (wparam_t)0x25U) { /* VK_LEFT */
            g_left_up = 1;
        }
        maybe_finish(hwnd);
        return 0;
