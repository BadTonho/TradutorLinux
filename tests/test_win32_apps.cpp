...
TEST(SevenZipGuiCoverageTest, AllApisAndModules) {
    ...
    ASSERT_NE(state_menu, nullptr);
    ASSERT_EQ(tl_AppendMenuA(state_menu, 0, 100, "State"), 1);
    EXPECT_EQ(tl_EnableMenuItem(state_menu, 100, 1), 0);
    EXPECT_EQ(tl_CheckMenuItem(state_menu, 100, 0x8U), 0U);
    EXPECT_EQ(tl_CheckMenuRadioItem(state_menu, 100, 100, 100, 0), 1);
    EXPECT_EQ(tl_DestroyMenu(state_menu), 1);
