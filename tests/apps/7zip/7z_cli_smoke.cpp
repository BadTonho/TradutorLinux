...
int main(const int argc, char** argv) {
    ...
    if (!std::filesystem::create_directories(staging / "extracted", error) || error) {
    ...
    constexpr std::string_view kPayload = "TradutorLinux 7-Zip CLI smoke\n";
    const std::filesystem::path input = staging / "input.txt";
    const std::filesystem::path archive = staging / "payload.zip";
    ...
    const auto create = run_runtime(
        ...
        {"a", "-tzip", to_guest_path(archive), to_guest_path(input)},
        staging / "create.stdout", staging / "create.stderr");
    const auto list = run_runtime(
        runtime, staged_executable, prefix,
        {"l", to_guest_path(archive)}, staging / "list.stdout", staging / "list.stderr");
    const auto extract = run_runtime(
        runtime, staged_executable, prefix,
        {"x", to_guest_path(archive), "-o" + to_guest_path(staging / "extracted"), "-y"},
        staging / "extract.stdout", staging / "extract.stderr");

    const bool extracted = read_text(staging / "extracted" / "input.txt") == kPayload;
    const bool ok = contains_loader_lifecycle(create, "Archive size:") &&
                    ...
                    contains_loader_lifecycle(extract, "Size:") && extracted;
    if (!ok) {
    ...
    std::cout << "7-Zip CLI create/list/extract: ok\n";
    std::filesystem::remove_all(staging, error);
    return 0;
}
