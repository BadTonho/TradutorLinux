...
[[nodiscard]] SmokeResult run_smoke(const std::filesystem::path& runtime,
    ...
    if (xvfb.pid <= 0 || xvfb.display.empty()) {
        ...
        std::cerr << "smoke do WinRAR: Xvfb indisponível; cenário ignorado\n";
    ...
    if (runtime_pid == 0) {
        ...
        ::close(trace_fd);
        ::execl(runtime.c_str(), runtime.c_str(), "--trace=runtime,gui,process", "--timeout",
                "6", "--cpu", "3", "--memory", "512", target.c_str(),
    ...
    if (display != nullptr) {
        for (int attempt = 0; attempt < 80 && window == 0; ++attempt) {
            window = find_window_by_name(display, DefaultRootWindow(display),
    ...
    passed = passed && trace.find("window-created backend=\"x11\" window_id=") !=
                           std::string::npos &&
             trace.find("caption=\"WinRAR self-extracting archive\"") != std::string::npos &&
    ...
    if (!passed) {
        std::cerr << "smoke do WinRAR não confirmou cancelamento controlado e saída esperada\n";
        std::cerr << trace;
    }
    std::filesystem::remove_all(staging, error);
...
int main(const int argc, char** const argv) {
    if (argc != 3) {
        std::cerr << "uso: winrar_sfx_smoke <runtime> <WinRAR_x64.exe>\n";
        return 2;
    }
