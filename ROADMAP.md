...
- **Próximo resultado observável:** validar `bzip2.exe` de ponta a ponta — todos os imports de KERNEL32 e msvcrt.dll estão implementados (100% compatibilidade), mas o executável trava durante a inicialização do CRT (`_initterm` ou constructor global); resolver esse bloqueio desbloqueia o golden test e o e2e. `dos2unix`/`unix2dos` dependem de `SHELL32.dll!CommandLineToArgvW` e de dezenas de APIs novas de `msvcrt.dll` (Fase 9+).
