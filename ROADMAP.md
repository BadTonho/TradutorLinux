...
- **Próximo resultado observável:** a próxima fronteira da GUI mínima ainda está por decidir — candidatos: tratamento de foco/teclado estendido (shift/maiúsculas reais, mais virtual keys), temporizadores (`WM_TIMER`), ou o primeiro desenho via GDI mínimo (`TextOut` + `WM_PAINT` com `HDC`). Decidir com fixture e teste de integração antes de implementar.
