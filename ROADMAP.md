...
- **Marco concluído:** a Fase 8 começou com a definição dos primeiros aplicativos-alvo reais, de código aberto e compilados em CI: `xxd` (vim `v9.2.0957`), `bzip2` (`1.0.8`) e `dos2unix`/`unix2dos` (`7.5.6`). O módulo `tests/targets` baixa as fontes pinadas por hash SHA-256, faz o cross-build com `mingw-w64` (opção `TL_BUILD_TARGET_APPS=ON`, job `target-apps` do CI) e protege os imports reais em manifests via `llvm-readobj` e `--report` (8 testes, label `targetapp`). Nenhum alvo executa ainda: todos importam `msvcrt.dll` (fora de escopo até a Fase 9) e o `--report` os classifica como `result: unsupported` / `execution: not-attempted`. Os imports capturados (xxd: 73 símbolos; bzip2: 69; dos2unix/unix2dos: 88, incluindo `SHELL32.dll!CommandLineToArgvW`) guiam o subconjunto mínimo de CRT da Fase 9.
- **Próximo resultado observável:** executar o primeiro aplicativo-alvo de ponta a ponta. O caminho mais curto é fechar o subconjunto de `msvcrt.dll` + `KERNEL32.dll` exigido pelo `xxd.exe` (o alvo mais simples): núcleo CRT do mingw (`__getmainargs`, `__iob_func`, `_fmode`, `_initterm`, stdio, strings, `malloc`/`free`) e as APIs de KERNEL32 que o CRT interno e o app exigem (`GetCommandLineA`, `GetStartupInfoA`, `GetEnvironmentStringsA`, critical sections, `TlsGetValue`, `VirtualProtect`/`VirtualQuery`, `MultiByteToWideChar`). Decidir com fixture/teste de integração antes de implementar; `dos2unix` fica para depois por exigir o caminho `W` e `SHELL32`.
