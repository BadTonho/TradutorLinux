...
| `winrar-x64-723.exe` | 179/251 | 72 | GUI/segurança e APIs pendentes | `unsupported` |
...
| `lghub_installer.exe` | 103/114 | 11 | segurança e arquivos pendentes | `unsupported` |
| `Rockstar-Games-Launcher.exe` | 223/338 | 115 | GUI/segurança/rede e APIs pendentes | `unsupported` |

Os totais de WinRAR, Logitech G HUB e Rockstar foram medidos novamente na Fase
13.8 somente com `--report`; os binários não foram executados. O contexto de
processo/console resolveu nove imports no WinRAR, onze no Logitech G HUB e dez
no Rockstar. Eles têm V2 e a forma aceita de `UWOP_SET_FPREG` classificados,
dispatcher SEH explícito, ambiente/locale/FLS determinísticos e console W, mas
retornam `5` (`Unsupported`) pelas APIs de arquivos, GUI, segurança e demais
...
9. [ ] **Fase 13.9 — enumeração e metadados de arquivos x64.** Depois,
   identidade/ACL,
...
| Resolvidos pelo runtime | 179/251 (130 estáticos + 49 atrasados), Fase 13.8 |
| Ausentes | 72 (26 estáticos + 46 atrasados), Fase 13.8 |
...
| Resultado do `--report` | Fase 13.8: `Unsupported`/exit `5` por imports ausentes; execução não tentada |
| Fonte | análise local de 2026-08-23 |

Na Fase 13.8, o `--report` leu V2 e classificou todos os 251 imports sem
executar o binário. Os 95 símbolos atrasados continuam inventariados por DLL,
com 49 já resolvidos. O contexto de processo/console elevou o total em nove
imports; WinRAR permanece `unsupported` sobretudo por GUI, segurança e APIs de
arquivos/sistema restantes.
...
| Resolvidos pelo runtime | 103/114, após a Fase 13.8 |
| Ausentes | 11, após a Fase 13.8 |
...
`--report` após a Fase 13.8, confirma 103 imports resolvidos sem executar o
...
Este caso confirmou que contexto de processo/console também era uma capacidade
compartilhada e foi promovido na Fase 13.8. A próxima lacuna direta é a camada
de enumeração/metadados de arquivos; ACLs continuam a exigir uma fixture de
segurança genérica antes de serem consideradas suporte ao instalador.
...
| Resolvidos pelo runtime | 223/338 (146 estáticos + 77 atrasados), Fase 13.8 |
| Ausentes | 115 (59 estáticos + 56 atrasados), Fase 13.8 |
...
Na Fase 13.8, o `--report` aceitou a extensão observada em
...
contexto de processo/console resolveram 29 imports compartilhados; além das
