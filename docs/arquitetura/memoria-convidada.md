...
| Buffers de saída | `ReadFile`/`WriteFile`, `FindFirstFileA/W`, `GetProcessMemoryInfo`, console/tempo, rede, locale, GUI e handles | Escrita em página desmontada ou somente leitura | `write_guest_memory` nos caminhos migrados; outras APIs ainda usam validação seguida de escrita direta |
