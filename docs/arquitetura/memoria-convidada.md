...
|---|---|---|---|
| Strings de entrada | `validate_mapped_cstring`, `validate_mapped_wstring`, `WININET` e `MPR` | Leitura após snapshot ou string sem terminador | Validação por blocos copiados com `read_guest_memory` nos caminhos migrados |
...
| Buffers de saída | Resultados de arquivos, rede, locale, GUI e handles | Escrita em página desmontada ou somente leitura | `write_guest_memory` nos caminhos migrados; outras APIs ainda usam validação seguida de escrita direta |
