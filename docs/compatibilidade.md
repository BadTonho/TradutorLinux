...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | `guest-timeout` exit `72` com limite de 3 s sob Xvfb | `RegisterClassExA` e inicialização do loop foram alcançados; a execução foi encerrada pelo limite externo do convidado | FLS 0/1 ok; não promover como suporte concluído |
...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `guest-signal` exit `71` sob Xvfb | a execução alcançou `startup-info` wide e terminou com SIGSEGV controlado em endereço nulo; não declarar suporte | a matriz B2 usou Xvfb; o resultado anterior sem display permanece apenas evidência ambiental |
...
selecionados sob Xvfb e limites controlados; a B3 testou apenas os candidatos
