...
| `tl_network_loopback.exe` | PE32+ AMD64 | Não | `WS2_32.dll` TCP/UDP, resolução local, `WSAPoll` e eventos WSA | Fixture somente loopback, com TCP, UDP e `localhost`; valida `WSAEventSelect`, `WSACreateEvent`, espera com timeout, `FD_ACCEPT` e `WSAEnumNetworkEvents`; passa com sockets permitidos e é skip controlado em sandbox que retorna `EACCES/EPERM` | WS2_32 |
...
`getaddrinfo` para `localhost`/loopback, conversões de ordem de bytes,
`WSAPoll` e o subconjunto de eventos WSA (`WSAEventSelect`, objetos manuais,
espera e enumeração de eventos). A fixture nunca acessa Internet; no sandbox
sem permissão de socket, o teste retorna um skip controlado, enquanto a
validação com loopback permitido passa de ponta a ponta.
...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | `guest-timeout` exit `72` no fluxo geral; smoke D2 da configuração termina com `exit 0` | `putty_smoke` sob Xvfb localiza `PuTTY Configuration`, envia `WM_DELETE_WINDOW` e confirma saída limpa em Rust ON/C++ OFF; o fluxo SSH completo continua não validado | FLS 0/1 ok; não promover como suporte geral |
...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `guest-timeout` exit `124` sob Xvfb válido | a corrupção de heap observada na C2 foi corrigida na D1: `lstrcpyW`/`lstrcpynW`/`lstrcmpW`/`lstrcmpiW` agora usam unidades guest UTF-16 de 16 bits; E2 fecha `Configurator`/`stylers.xml` e reproduz `cxx-throw ignored` seguido de `guest-signal` exit `71` em Rust ON/OFF | não declarar suporte GUI: exceções C++/SEH e o fluxo principal continuam fora do contrato; o timeout sem interação também permanece |
...
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Parcial: configuração | C2 do fluxo geral termina em `GuestTimeout 72`; D2 abre `PuTTY Configuration`, envia `WM_DELETE_WINDOW` e termina com `exit 0` em Rust ON/C++ OFF; o smoke pode ser registrado no CTest com o corpus; SSH completo ainda não é suportado |
...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | D1 corrigiu a ABI guest UTF-16 das APIs `lstr*W`; sob Xvfb válido Rust ON/C++ OFF chegam à janela `Configurator` e terminam em timeout controlado `124`; o smoke de bloqueio controlado pode ser registrado no CTest com o corpus; ainda falta interação/encerramento GUI |
