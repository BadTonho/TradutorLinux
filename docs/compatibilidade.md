...
| 5 | `WinRAR_x64.exe` `winrar-x64-723.exe` | PE32+ x86-64 | 251/251 (100%) | `supported` | `ExitProcess 0` no cenário SFX sem GUI; sob Xvfb, o smoke de cancelamento termina com `ExitProcess 0` após `WM_DELETE_WINDOW`, enquanto a execução sem interação termina em `guest-timeout 72` | delay `GDI32/ADVAPI32/SHELL32/ole32`; `IDOK` materializa apenas a etapa inicial observada e ainda termina com exit `2`; extração completa e uso diário continuam não validados |
