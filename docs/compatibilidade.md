...
| 5 | `WinRAR_x64.exe` `winrar-x64-723.exe` | PE32+ x86-64 | 251/251 (100%) | `supported` | `ExitProcess 0` no cenário SFX sem GUI; sob Xvfb, o smoke de cancelamento termina com `ExitProcess 0` após `WM_DELETE_WINDOW`; a interação `Return` alcança a enumeração e o unwind convidado, mas termina com `guest-signal 71` | delay `GDI32/ADVAPI32/SHELL32/ole32`; o contrato genérico de `RtlUnwindEx` foi ampliado, mas a ponte de pilha/contexto necessária à extração completa ainda não está validada; uso diário continua não validado |
