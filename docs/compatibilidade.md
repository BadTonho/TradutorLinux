...
| `tl_d3d12_probe.exe` | PE32+ AMD64 | Não | `D3D12.dll!D3D12CreateDevice`; `KERNEL32.dll!ExitProcess`, `GetStdHandle`, `WriteFile` | **Fixture VKD3D-Proton controlada da B14.6.5:** cria dispositivo D3D12, fila direta, allocator, command list, fence e sinalização; `integration_proton_d3d12` passa com Proton Experimental real sob Xvfb, preserva stdout (`D3D12 command path ready`) e retorna `0`. Valida somente inicialização e submissão básica D3D12→VKD3D-Proton/Vulkan; não declara swap chain, D3D12 completo, áudio, entrada ou jogos | B14.6.5 |
...
(`Unsupported`) sem executar o runtime nativo. A fixture
`tl_graphics_probe.exe` e `integration_proton_graphics` acrescentam um piloto
real controlado: sob Xvfb e Proton Experimental instalado em
`TL_PROTON_ROOT`, o fluxo D3D11 cria uma janela, apresenta um frame e retorna
`0`, mantendo os streams e o prefixo isolado. Essa evidência é limitada ao
caminho D3D11→DXVK/Vulkan em X11; não promove D3D12/VKD3D-Proton, áudio,
entrada ou qualquer jogo. Roblox permanece apenas como alvo exploratório até
existir execução reproduzível com suas limitações publicadas.

O teste `integration_proton_d3d12` acrescenta a validação real controlada do
primeiro caminho D3D12: a fixture cria recursos de execução e sinaliza uma
fila, retornando `0` sob Proton Experimental. A evidência é restrita à
inicialização/submissão básica D3D12→VKD3D-Proton/Vulkan; não representa
suporte completo a D3D12 ou a aplicações do catálogo.
