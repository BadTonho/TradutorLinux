...
| **7-Zip CLI (`7z_x64.exe`)** | PE32+ x86-64 | 100% (133/133) | Suportado no subconjunto | Smoke isolado cria, lista e extrai ZIP `stored`, ZIP `DEFLATE` e arquivo `7z` com `LZMA2` pela `7z.dll` carregada do diretório da aplicação; exit `0` em Rust ON/C++ OFF |
