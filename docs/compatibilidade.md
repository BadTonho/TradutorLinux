...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `guest-timeout` exit `124` no fluxo sem interação; smoke termina com `ExitProcess(3)` | a corrupção de heap da C2 foi corrigida na D1; em F2, `shlwapi` (`PathFileExistsA/W`, `PathIsDirectoryA/W`) passou a usar resolução canônica (`translate_windows_path`), eliminando o erro de `Load stylers.xml failed`. O smoke fecha `Configurator` e falha se esse erro reaparecer; o processo avança até a verificação de certificados digitais (`WinVerifyTrust` com `WTD_CHOICE_FILE`), onde termina controladamente em `ExitProcess(3)` | não declarar suporte GUI: `WinVerifyTrust` para arquivos e exceções C++/SEH fora do `FuncInfo` v3 continuam como gates e o fluxo principal permanece não validado |
...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | D1 corrigiu a ABI guest UTF-16 das APIs `lstr*W`; F2/F3 normalizou a resolução de caminhos em `shlwapi` (`PathFileExistsA/W`) e as pastas especiais de `shell32` (`SHGetKnownFolderPath`, `SHGetFolderPathW`) para caminhos Windows canônicos. O executável avança pelas janelas de configuração e estilização até a validação de certificados de plugins/módulos em `WinVerifyTrust(WTD_CHOICE_FILE)`, terminando em `ExitProcess(3)` controlado; fluxo principal permanece não validado |
