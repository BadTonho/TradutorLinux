...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | tentativa `guest-timeout` exit `72` com display `:99` indisponível | `RegisterClassExA` foi alcançado, mas `CreateWindowExA` registrou `x11/connect-failed`; o timeout não é evidência de execução GUI válida | FLS 0/1 ok; não promover como suporte concluído |
...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | tentativa `guest-signal` exit `71` com X11 indisponível | a execução alcançou `startup-info` wide e terminou com SIGSEGV controlado em endereço nulo; sem X11 válido, não declarar suporte GUI | a matriz B2 não substitui a necessidade de um Xvfb iniciado com sucesso |
...
selecionados com limites controlados; a tentativa de Xvfb falhou antes dos
casos GUI e foi registrada como skip ambiental. A B3 testou apenas os candidatos
PE32+ e MSIX em prefixos temporários. Os instaladores PE32/x86, DLLs e o
HWiNFO empacotado não foram executados como aplicativos.
...
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Não concluído | B2 alcançou o entry point e FLS, mas `CreateWindowExA` registrou `x11/connect-failed` e o limite encerrou em `GuestTimeout 72`; repetir com Xvfb válido |
...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | B2 tentou execução com X11 indisponível e terminou em SIGSEGV controlado `71`; exige repetição sob Xvfb iniciado com sucesso |
