...
| **7-Zip CLI (`7z_x64.exe`)** | PE32+ x86-64 | 100% (133/133) | Suportado no subconjunto | Smoke isolado cria, testa, remove e atualiza ZIP `stored`, além de criar/listar/extrair ZIP `DEFLATE` e arquivo `7z` com `LZMA2`, pela `7z.dll` carregada do diretório da aplicação; preserva `input-data/café-日本.txt`, verifica `updated.txt`, exercita caminhos de staging com espaços e termina com exit `0` em Rust ON/OFF |
