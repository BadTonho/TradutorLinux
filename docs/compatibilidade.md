...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | `guest-timeout` exit `72` sob Xvfb válido | `RegisterClassExA`/`CreateWindowExA` criaram `PuTTYTimerWindow`; não houve `x11/connect-failed`, mas falta cenário automatizado de interação/encerramento | FLS 0/1 ok; não promover como suporte concluído |
...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `guest-signal` exit `71` sob Xvfb válido | a execução alcançou `startup-info` wide e terminou com SIGSEGV controlado em endereço nulo; o resultado reproduzido com X11 válido é um bloqueio real a investigar | não declarar suporte GUI até isolar a primeira operação após o startup |
...
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Não concluído | C2 sob Xvfb válido criou `PuTTYTimerWindow` e terminou em `GuestTimeout 72`; falta cenário automatizado de interação/encerramento |
...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | C2 reproduziu SIGSEGV controlado `71` após `startup-info` wide sob Xvfb válido; bloqueio real a investigar |
