...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `guest-timeout` exit `124` no fluxo sem interação; smoke E45 termina com `ExitProcess(3)` | a corrupção de heap observada na C2 foi corrigida na D1: `lstrcpyW`/`lstrcpynW`/`lstrcmpW`/`lstrcmpiW` agora usam unidades guest UTF-16 de 16 bits; E45 fecha `Configurator`/`stylers.xml`, classifica os handlers estáticos sem `FuncInfo` como fora do subconjunto C++ e evita a transferência de cleanup inválida | não declarar suporte GUI: exceções C++/SEH fora do `FuncInfo` v3 continuam rejeitadas de forma controlada e o fluxo principal permanece não validado |
...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | D1 corrigiu a ABI guest UTF-16 das APIs `lstr*W`; o fluxo sem interação continua em timeout `124`. No smoke E45, Rust ON/C++ OFF chegam à janela `Configurator`, fecham `stylers.xml` e terminam em `ExitProcess(3)` com rejeição SEH controlada; ainda falta interação/encerramento do fluxo principal |
