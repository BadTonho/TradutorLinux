...
| **Notepad++ (`notepad++.exe`)** | PE32+ x86-64 | 100% (584/584) | Não suportado como fluxo concluído | D1 corrigiu a ABI guest UTF-16 das APIs `lstr*W`; F2 corrigiu a resolução de caminhos em `shlwapi` (`PathFileExistsA/W`), eliminando o popup de erro `Load stylers.xml failed` e a janela `Configurator`. O executável agora avança até a validação de assinatura digital de arquivos em `WinVerifyTrust(WTD_CHOICE_FILE)`, terminando em `ExitProcess(3)` controlado; fluxo principal permanece não validado |
