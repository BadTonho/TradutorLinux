...
O probe local do PuTTY mantém essa distinção: as etapas E39–E43 validam o
transporte inicial de loopback, as notificações `WSAAsyncSelect` e `FD_CLOSE`,
`SSH_MSG_KEXINIT` observado e a sequência controlada `SSH_MSG_IGNORE` +
`SSH_MSG_DISCONNECT`, mas o fluxo ainda termina em
