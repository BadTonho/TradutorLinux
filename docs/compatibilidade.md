...
|---|---|---|---:|---|---|---|
| 1 | `7z_x64.exe` | PE32+ x86-64 | 133/133 (100%) | `supported` | `ExitProcess 0`; listagem de ZIP por `7z.dll` com stdout Rust/C++ idêntico | DLL PE ao lado do executável é encontrada, mapeada, anexada e descarregada; operações além da listagem continuam não validadas |
...
| **7-Zip CLI (`7z_x64.exe`)** | PE32+ x86-64 | 100% (133/133) | Suportado no subconjunto | Executou o banner e listou um ZIP `stored` pela `7z.dll` carregada do diretório da aplicação; stdout e exit `0` foram iguais em Rust ON/C++ OFF |
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Parcial: configuração | C2 do fluxo geral termina em `GuestTimeout 72`; D2 abre `PuTTY Configuration`, envia `WM_DELETE_WINDOW` e termina com `exit 0` em Rust ON/C++ OFF; SSH completo ainda não é suportado |
