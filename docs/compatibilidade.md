...
|---|---|---:|---|---|
| **7-Zip File Manager (`7zFM_x64.exe`)** | PE32+ x86-64 | 100% (298/298) | Shell visual experimental; não suportado como fluxo GUI concluído | Menu de classe MENUEX real, dropdowns de primeiro nível, seleção básica por mouse/teclado de itens folha, toolbar orientada pelos `idCommand` reais, endereço editável restrito à raiz visual, navegação lateral interativa, lista selecionável com navegação visual por pastas via Enter ou duplo clique e status aparecem; `WM_COMMAND` básico pode ser encaminhado, mas operações de arquivo, submenus aninhados e navegação do convidado ainda não estão ligados |
