...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `ExitProcess(0)` no smoke real `notepadpp_fh4_headless_smoke` | D1/F2/F3 preservam a inicialização sem `Load stylers.xml failed`; R6 decodifica FH4 host-side, seleciona dois `catch` tipados, materializa capturas por referência e retorna pelo `catchret` com a pilha alinhada. R7 executa somente o prefixo seguro de quatro cleanups e registra `fh4-cleanup-limit` para a cadeia maior observada; o caminho validado não produz `guest-signal`/`guest-timeout`; o smoke GUI continua separado e pode ser ignorado quando Xvfb não sobe | parcial: inicialização headless e C++ EH FH4 limitado; interação GUI completa, cadeias FH4 arbitrárias, `WinVerifyTrust` efetivo e uso diário permanecem não validados |
