...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `ExitProcess(0)` no smoke real `notepadpp_fh4_headless_smoke` | D1/F2/F3 preservam a inicialização sem `Load stylers.xml failed`; R6 decodifica FH4 host-side, seleciona dois `catch` tipados, materializa capturas por referência e retorna pelo `catchret` com a pilha alinhada. R26 isola o matching do frame alvo para C++ EH no establisher frame, delimita o plano de destruição ao `target_state` (`try_low`) e amplia o limite seguro de cleanups para 16 ações; R34 implementa `WM_NCCREATE` (0x0081) antes de `WM_CREATE` e registra handles de janelas filhas antes de acionar suas wndprocs, eliminando falhas de `GWLP_USERDATA` (nullptr) e permitindo o registro e instanciação de controles Scintilla, ListBoxX e CallTip na janela principal X11 | parcial: inicialização headless e C++ EH FH4 estendido; ciclo de vida Win32 (WM_NCCREATE/WM_CREATE) operacional; interação GUI completa, cadeias FH4 arbitrárias, `WinVerifyTrust` efetivo e uso diário permanecem não validados |
