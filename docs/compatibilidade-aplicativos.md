...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `ExitProcess(0)` no smoke real `notepadpp_fh4_headless_smoke` | D1/F2/F3 preservam a inicialização sem `Load stylers.xml failed`; R6 decodifica FH4 host-side, seleciona dois `catch` tipados, materializa capturas por referência e retorna pelo `catchret` com a pilha alinhada. O caminho validado não produz `guest-signal`/`guest-timeout`; o smoke GUI continua separado e pode ser ignorado quando Xvfb não sobe | parcial: inicialização headless e C++ EH FH4; cleanups FH4, interação GUI completa, `WinVerifyTrust` efetivo e uso diário permanecem não validados |
