...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | `guest-timeout` exit `72` no fluxo geral e no probe SSH local | D2 abre/fecha `PuTTY Configuration`; E31 preenche host/porta, aciona `Open` e alcança a janela principal `PuTTY`; E33 registra `WSAStartup`, mas nenhuma chamada convidada de `getaddrinfo`/`socket`/`connect`/`send`/`recv`, e o listener local recebe zero bytes em Rust ON/C++ OFF; E34 confirma `DispatchMessageA` antes de `GetMessageA` ocioso; E35 mostra que, depois da janela de sessão, o convidado fica ativo em CPU sem voltar ao message loop; E36 captura o RIP do timeout e o correlaciona com `__cyg_profile_func_enter` do runtime, fora da imagem PE; E37 corrige a reentrância do gravador `--trace-json` sem alterar o resultado do aplicativo. O SSH completo continua não validado; FLS 0/1 ok; não promover como suporte geral |
...
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Parcial: configuração e janela de sessão; conexão não alcançada | C2 do fluxo geral e E31 terminam em `GuestTimeout 72`; D2 abre/fecha `PuTTY Configuration`, enquanto o probe preenche host/porta, aciona `Open`, alcança a janela `PuTTY`, observa `DispatchMessageA` antes do `GetMessageA` ocioso e zero bytes enviados em Rust ON/C++ OFF; E35 confirma que o convidado fica em execução ativa após criar a janela, sem voltar ao message loop; SSH completo ainda não é suportado |
