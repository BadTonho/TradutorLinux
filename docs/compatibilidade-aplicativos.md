...
| `tl_peek.exe` | PE32+ AMD64 | Não | `KERNEL32.dll!ExitProcess`; `USER32.dll!RegisterClassExA`, `CreateWindowExA`, `ShowWindow`, `PeekMessageA`, `DefWindowProcA` | Fixture do contrato não bloqueante: recebe `WM_KEYDOWN('Q')`, confirma `PM_NOREMOVE` (`remove=0`) e depois `PM_REMOVE` (`remove=1`), terminando com exit `7`; validada no cenário `peek` do `runtime_gui_smoke`. Filtros de mensagem continuam fora do subconjunto | Fase 7 / R9 |
...
| `tl_7zfm_gui.exe` | PE32+ AMD64 | msvcrt | `KERNEL32`, `USER32`, `ADVAPI32`, `SHELL32`, `COMCTL32`, `MPR`, `msvcrt` | **Suportado no subconjunto:** valida menus, notificações de arquivo, drag & drop, shell folders e a rejeição controlada das operações MPR; não cria conexões nem handles de rede; imprime `7ZFM GUI APIS OK\n`, exit `0` | 7-Zip GUI |
...
| 2 | `7zFM_x64.exe` | PE32+ x86-64 | 298/298 (100%) | smoke Xvfb de `tests/apps/7zip/seven_zip_smoke.cpp` com `app run`, perfil schema 4 explícito e exit `0`; tentativa B2 não revalidada por X11 indisponível | smoke externo cadastra o aplicativo como `7zip`, prepara `compat/profile.json`, confirma `compat-profile` carregado e `compat-extension` `7zip`, seleciona `input.txt`, aciona `Copy` (`idCommand=546`), verifica o arquivo copiado e encerra o runtime com exit `0` | janela X11 abre com shell visual host-side da extensão `compat/apps/7zip/`; a tentativa B2 registrou falha de inicialização do Xvfb antes do aplicativo e não altera essa conclusão; menu de classe `RT_MENU`/MENUEX real (6 itens de nível superior), dropdowns aninhados e seleção básica por mouse/teclado de itens folha encaminham `WM_COMMAND`; lista imediata do diretório do executável é selecionável, recebe hover e permite navegação visual por pastas com Enter ou duplo clique; árvore lateral recebe hover, retorna à raiz e seleciona diretórios Linux conhecidos; barra `Address` permite navegar somente dentro da raiz visual; toolbar segue os `idCommand` reais, recebe hover, mostra pressão e cancela soltura fora do botão; `Copy` (`idCommand=546`) copia, de forma opt-in, um arquivo selecionado para destino existente dentro da raiz, sem sobrescrever; imports MPR permanecem resolvidos como stubs e a fixture confirma rejeição controlada, sem alegar suporte a rede | geometria inválida normalizada para `800x600`; limite da lista em 128 linhas; `MPR.dll` 6/6 com `runtime-support: stub`; demais operações ainda não concluídas |
...
| 4 | `putty_x64.exe` | PE32+ x86-64 | 348/348 (100%) | `guest-timeout` exit `72` no fluxo geral e no probe SSH local | D2 abre/fecha `PuTTY Configuration`; E31 preenche host/porta, aciona `Open` e alcança a janela principal `PuTTY`; E33–E38 registram a transição GUI, corrigem o diagnóstico e separam o custo do trace; E39–E45, no `build/debug`, confirmam `WSAAsyncSelect` com `FD_CONNECT`/`FD_WRITE`/`FD_READ`/`FD_CLOSE`, `send` do banner, `recv`, `SSH_MSG_KEXINIT` do cliente, seleção de algoritmos comuns, `SSH_MSG_KEXINIT` controlado do servidor e a sequência `SSH_MSG_IGNORE` + `SSH_MSG_DISCONNECT`. O fluxo termina em `guest-timeout 72` depois do diálogo de erro; SSH completo continua não validado e não deve ser promovido como suporte geral |
...
| 6 | `Rufus_x64.exe` | PE32+ x86-64 | — | `malformed` exit `4` | `not-attempted` | O diretório `.pdata` aponta para `RVA 0xc5000`, dentro da seção virtual-only `UPX0` (`SizeOfRawData=0`); Rust e C++ rejeitam a imagem antes do mapeamento. O desempacotamento necessário para materializar a tabela permanece fora do escopo |
...
| 8 | `RobloxPlayerInstaller.exe` | PE32+ x86-64 | 430/430 (100%) | `supported (Worker inicializado)` | `guest-timeout 72` (executa sem falhas até o timeout); suporte a `STACK_SIZE_PARAM_IS_A_RESERVATION` em `CreateThread` superou o `RBXCRASH FatalRuntimeError Worker,28` (R32) | A thread de Worker inicia com sucesso (`creation-flags=0x10000`), executa sincronização Win32 e captura exceções tipadas FH4 C++ EH sem falhas |
| 9 | `Rockstar-Games-Launcher.exe` | PE32+ x86-64 | 338/338 (100%) | `supported (GUI inicializada)` | Inicialização do Breakpad/Crashpad concluída via `rpcrt4.dll!UuidCreate` e `kernel32!AppPolicyGet*`; cria janela X11 de 480x180 e mapeia na tela (`window_id="77594625"`, `window-mapped`) (R31) | O executável inicializa Breakpad/Crashpad com sucesso, resolve UuidCreate e políticas de desktop clássico, conecta ao servidor X11 host (`display=:0.0`), registra classe de janela, cria e mapeia janela de inicialização de 480x180 sem falhas ou asserts |
...
| 10 | `Logitech_GHUB_x64.exe` `lghub_installer.exe` | PE32+ x86-64 | 114/114 (100%) | `supported` | `ExitProcess 0` (R30) | bootstrapper PE32+ extrai `lghub_setup.exe` de `.rsrc`, configura permissões de DACL de forma isolada no prefixo (`SetEntriesInAclW`/`SetNamedSecurityInfoW`), despacha processo via `ShellExecuteExW`/`CreateProcessW` e conclui limpeza de diretório temporário com código 0 |
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `ExitProcess(0)` no smoke real `notepadpp_fh4_headless_smoke` | D1/F2/F3 preservam a inicialização sem `Load stylers.xml failed`; R6 decodifica FH4 host-side, seleciona dois `catch` tipados, materializa capturas por referência e retorna pelo `catchret` com a pilha alinhada. R26 isola o matching do frame alvo para C++ EH no establisher frame, delimita o plano de destruição ao `target_state` (`try_low`) e amplia o limite seguro de cleanups para 16 ações; R34 implementa `WM_NCCREATE` (0x0081) antes de `WM_CREATE` e registra handles de janelas filhas antes de acionar suas wndprocs, eliminando falhas de `GWLP_USERDATA` (nullptr) e permitindo o registro e instanciação de controles Scintilla, ListBoxX e CallTip na janela principal X11; R35 corrige `SendMessageA/W` para retornar `LRESULT` de 64 bits e despachar mensagens para `wndproc` de janelas filhas (resolvendo `SCI_GETDIRECTFUNCTION` para as 4 instâncias de Scintilla), expande o limite de janelas para 256 e classes para 64, e remove a restrição que impedia controles de terem janelas filhas como pai (eliminando a falha de `Splitter::init` e superando completamente a fase de criação `WM_CREATE` da janela principal); R36 implementa suporte completo a templates binários estendidos `DLGTEMPLATEEX` (0xFFFF0001, helpID, estilos, fonte com weight/italic/charset e controles com IDs 32-bit), expande slots para 512 janelas e 256 controles por diálogo e remove restrições de controle pai em diálogos (docking managers), criando com sucesso todos os 5 diálogos estáticos do Notepad++ e ingressando no loop de mensagens principal (`GetMessageA`/`DispatchMessageA`) | parcial: inicialização headless e C++ EH FH4 estendido; ciclo de vida Win32 (WM_NCCREATE/WM_CREATE), Scintilla e hierarquia de splitters operacionais; interação GUI completa, cadeias FH4 arbitrárias, `WinVerifyTrust` efetivo e uso diário permanecem não validados |
| 11a | `Notepad++/updater/GUP.exe` | PE32+ x86-64 | 149/153 (97% direto; 100% com `libcurl.dll` 193/193) | `supported` | `ExitProcess 4294967295` (sem parâmetros de update) | o `libcurl.dll` local é encontrado, mapeado e possui 100% de imports resolvidos (WLDAP32, CRYPT32, ADVAPI32, Normaliz, bcrypt); a imagem executa o entry point e encerra via `ExitProcess` |
...
| 13 | `Affinity x64.msix` | Zip/MSIX ZIP64 | 100% manifesto | `supported` | exit `0` (`--report` / `install`) | O parser reconhece o manifesto AppX (Canva.Affinity v3.2.3.4646, App\Affinity.exe), suporta pacotes de até 4 GiB e trata entradas DEFLATE vazias com segurança; a extração e o cadastro no catálogo do runtime são concluídos com sucesso. A execução funcional do aplicativo completo continua sujeita aos limites de subsistemas externos |
...
|---|---|---:|---|---|
| **7-Zip File Manager (`7zFM_x64.exe`)** | PE32+ x86-64 | 100% (298/298) | Fluxo principal restrito | A extensão host-side `7zip` é ativada somente por perfil schema 4 explícito; o smoke `tests/apps/7zip/seven_zip_smoke.cpp` cadastra e executa via `app run`, confirma o trace de perfil/extensão, exibe menu MENUEX real, dropdowns aninhados, toolbar orientada pelos `idCommand` reais com captura de pressão e hover, endereço editável restrito à raiz visual, navegação lateral interativa com hover, lista selecionável com hover e navegação visual por pastas via Enter ou duplo clique e status; `WM_COMMAND` básico pode ser encaminhado; o smoke seleciona um arquivo e conclui `Copy` (`546`) dentro da raiz, sem sobrescrever; demais operações continuam limitadas |
...
| **PuTTY SSH Client (`putty_x64.exe`)** | PE32+ x86-64 | 100% (348/348) | Parcial: configuração, janela de sessão e transporte inicial de loopback | C2/E31 preservam a limitação geral; D2 abre/fecha `PuTTY Configuration`, enquanto o probe preenche host/porta, aciona `Open`, alcança a janela `PuTTY` e E39–E45 confirmam `FD_CONNECT`/`FD_WRITE`/`FD_READ`/`FD_CLOSE`, `send` do banner, `SSH_MSG_KEXINIT` observado, seleção de algoritmos comuns, `SSH_MSG_KEXINIT` controlado do servidor, a sequência `SSH_MSG_IGNORE` + `SSH_MSG_DISCONNECT` e diálogo de erro controlado em `build/debug`; **E46 (R25, 2026-09-19)**: o PuTTY enviou `SSH_MSG_KEXDH_INIT` (tipo 30) com valor público DH group14 de 2048 bits — `parse_kexdh_init_packet` validou mpint `e` (256–258 bytes); probe reporta `KEXDH_INIT received, guest-timeout 72`; assinatura, `KEXDH_REPLY`, geração de chave de sessão e autenticação continuam não implementados; SSH completo não é suportado |
...
| **Roblox Player Installer (`RobloxPlayerInstaller.exe`)** | PE32+ x86-64 | 100% (430/430) | Parcial: Worker inicializado e SEH/C++ EH operacionais | STACK_SIZE_PARAM_IS_A_RESERVATION (`0x00010000`) aceito em `CreateThread` (R32) eliminou o panic de Worker (`RBXCRASH`); threads de Worker executam sincronização e C++ EH FH4 sem crash até timeout controlado |
...
| **Rockstar Games Launcher (`Rockstar-Games-Launcher.exe`)** | PE32+ x86-64 | 100% (338/338) | Parcial: inicialização e GUI mapeada | Inicialização do Breakpad/Crashpad completada com sucesso via `rpcrt4.dll!UuidCreate` e `kernel32!AppPolicyGet*` (R31); conecta ao display X11 host, registra classe Win32, cria e mapeia janela gráfica de 480x180 (`window-mapped`); fluxo de rede/login posterior não validado |
