...
| `tl_gdiex.exe` | PE32+ AMD64 | Não | `GDI32.dll` — `CreateFontW`/`SetDCBrushColor`/`SetDCPenColor`; `gdiplus.dll` — 8 APIs stub; `UxTheme.dll` — `SetWindowTheme`; `WINMM.dll` — `timeSetEvent`; `dbghelp.dll` — `SymFromAddr` | Fixture GDI estendido: valida GDI+ como rejeição controlada (status `1`, saídas zeradas, sem ponteiros host), além de `CreateFontW` wide, `SetDCBrush/PenColor`, `SetWindowTheme`, `timeSetEvent` `1` e `SymFromAddr` stub; `USER32` `GetDC`; saída `gdiex\n`, exit `0` | GDI estendido |
