...
| 11 | `notepad++.exe` | PE32+ x86-64 | 584/584 (100%) | `ExitProcess(0)` no smoke real `notepadpp_fh4_headless_smoke` | D1/F2/F3 preservam a inicialização sem `Load stylers.xml failed`; R6 decodifica FH4 host-side, seleciona dois `catch` tipados, materializa capturas por referência e retorna pelo `catchret` com a pilha alinhada. R26 isola o matching do frame alvo para C++ EH no establisher frame, delimita o plano de destruição ao `target_state` (`try_low`) e amplia o limite seguro de cleanups para 16 ações; o Notepad++ executa a cadeia completa de cleanups (`fh4-termination-cleanup`) sem atingir `fh4-cleanup-limit` e conclui ambos os catches com `ExitProcess(0)`; o smoke GUI continua separado e pode ser ignorado quando Xvfb não sobe | parcial: inicialização headless e C++ EH FH4 estendido; interação GUI completa, cadeias FH4 arbitrárias, `WinVerifyTrust` efetivo e uso diário permanecem não validados |
