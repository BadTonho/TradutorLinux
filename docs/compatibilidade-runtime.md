...
publicados junto com o bloco completo. A auditoria dos consumidores comuns foi
concluída: APIs não usam `validate_mapped_range` como garantia de acesso
posterior. As exceções são o predicado advisory mantido no validador e os
contratos especiais documentados para dados locais do despachante SEH/C++ e
endereços de callback dentro da imagem ativa.
...
| `USER32.dll` | `GetDlgItemTextA/W` / `GetDlgItemInt` | Suportado no subconjunto | Stubs retornam valores vazios/determinísticos e publicam texto/indicador por cópias protegidas |
...
| `COMCTL32.dll` | `TaskDialog*` / `ImageList_GetIconSize` / `ImageList_GetImageInfo` / `LoadIconWithScaleDown` | Suportado no subconjunto | Stubs e image lists preservam os retornos determinísticos; saídas de botões, dimensões, informações e ícones usam cópias protegidas |
| `COMDLG32.dll` | `GetOpenFileNameA/W` / `GetSaveFileNameA/W` / `ChooseColorA/W` / `ChooseFontA/W` / `PrintDlgW` / `CommDlgExtendedError` | Não suportado controlado | Não abre diálogos nem fabrica seleção; as funções de diálogo leem o prefixo da estrutura por cópia protegida, retornam `FALSE` e definem `ERROR_NOT_SUPPORTED`/`CDERR_DIALOGFAILURE` quando o ponteiro é válido; ponteiro nulo ou inacessível retorna `ERROR_INVALID_PARAMETER`/`CDERR_STRUCTSIZE` |
...
| `KERNEL32.dll` | `GetModuleHandleExA/W` | Suportado | Flags `PIN`/`UNCHANGED_REFCOUNT`/`FROM_ADDRESS`; `FROM_ADDRESS` aceita `0x1000` ou endereço dentro da imagem (`g_guest_image_base/size`); publica `phModule` por cópia protegida; erro `ERROR_INVALID_PARAMETER`/`FILE_NOT_FOUND` |
...
| `version.dll` | `GetFileVersionInfoSizeA/W` / `GetFileVersionInfoA/W` / `GetFileVersionInfoSizeExA/W` / `GetFileVersionInfoExA/W` / `VerQueryValueA/W` | Não suportado controlado | As dez exports são `ExportSupport::Stub`: não inventam tamanho, bloco `RT_VERSION` ou ponteiro host; consultas válidas retornam falha com `ERROR_NOT_SUPPORTED`, zeram `GetFileVersionInfoSize`/`VerQueryValue`/`len` por cópias protegidas e não sondam o bloco de dados que nunca é consumido |
| `PSAPI.dll` | `EnumProcesses` / `EnumProcessModules` / `EnumProcessModulesEx` / `GetModuleBaseNameA/W` / `GetModuleFileNameExA/W` / `GetProcessMemoryInfo` | Suportado no subconjunto | Enumera o processo atual e um módulo token; nomes, caminhos, contadores e tamanhos são publicados por cópias protegidas, sem validação prévia por snapshot de mapas |
...
| `DWMAPI.dll` | `DwmSetWindowAttribute` / `DwmGetWindowAttribute` / `DwmIsCompositionEnabled` / `DwmExtendFrameIntoClientArea` / `DwmEnableBlurBehindWindow` / `DwmFlush` / `DwmGetColorizationColor` | Não suportado controlado | Não cria composição Windows nem inventa estado visual: exports são `ExportSupport::Stub`, retornam `E_NOTIMPL`, limpam saídas válidas por cópia protegida e definem `ERROR_NOT_SUPPORTED`; `DwmDefWindowProc` permanece `Limited`, zera `lresult` por cópia protegida e retorna `FALSE`/não tratado |
| `KERNEL32.dll` | `WaitOnAddress` / `WakeByAddressSingle` / `WakeByAddressAll` | Suportado | `WaitOnAddress` compara cópias protegidas de `Address` e `CompareAddress` (`size` 1/2/4/8, alinhado); se diferente retorna `1`; se igual espera por `Wake*` ou `dwMilliseconds` (`INFINITE`→`wait`, `0`→timeout imediato) via `mutex`+`cv`+`version` por endereço; `WakeSingle`→`notify_one`, `WakeAll`→`notify_all`; timeout → `0` + `ERROR_TIMEOUT` (1460); exposto via `KERNEL32` e `api-ms-win-core-synch-l1-2-0.dll` (forwarder) |
...
| `KERNEL32.dll` | `ReadDirectoryChangesW` / `FlushViewOfFile` | Suportado no subconjunto de arquivos e mapeamentos | `ReadDirectoryChangesW` publica `lpBytesReturned` por cópia protegida e mantém o comportamento limitado do stub; `FlushViewOfFile` valida alinhamento e delega a validação do intervalo ao `msync(2)` do kernel; I/O overlapped e mapeamentos arbitrários não fazem parte do contrato |
...
| `KERNEL32.dll` | `GetVolumeInformationA/W` / `GetFileAttributesExW` / `GetFileTime` / `SetFileTime` / `GetFileInformationByHandle` / `GetFileInformationByHandleEx` | Suportado no subconjunto | Metadados determinísticos ou derivados de `stat`/`fstat`; entradas e saídas usam cópias protegidas, sem pré-validação por snapshot de `/proc/self/maps`; o horário de criação é lido para validar o ponteiro, mas não é alterado no Linux |
...
| `gdiplus.dll` | `GdiplusStartup` / `GdiplusShutdown` / `GdipAlloc` / `GdipFree` / `GdipCreateBitmapFromStream` / `GdipCloneImage` / `GdipDisposeImage` / `GdipCreateHBITMAPFromBitmap` | Não suportado controlado | As oito exports são `ExportSupport::Stub`: não inicializam GDI+, não alocam memória nem fabricam imagens/HBITMAPs; saídas válidas são zeradas por cópia protegida e as operações retornam `GenericError` (`1`) com `ERROR_NOT_SUPPORTED` |
| `UxTheme.dll` | `SetWindowTheme` / `OpenThemeData` / `CloseThemeData` / desenho, consultas e buffered paint | Não suportado controlado | As exports são `ExportSupport::Stub`: não criam tema, brush, HDC ou buffered-paint; retornam `E_NOTIMPL`, limpam saídas válidas por cópia protegida e definem `ERROR_NOT_SUPPORTED`; entradas inválidas retornam `E_INVALIDARG` |
| `WINMM.dll` | `timeSetEvent` | Suportado | Stub retorna `1` |
| `WINMM.dll` | `timeGetDevCaps` | Suportado no subconjunto | Publica capacidades determinísticas (`period_min=1`, `period_max=1000000`) por cópia protegida; tamanho insuficiente ou ponteiro inacessível retorna `TIMERR_NOCANDO` |
| `dbghelp.dll` | `SymFromAddr` | Não suportado controlado | Copia `SizeOfStruct` do buffer opcional `symbol`, zera `displacement` por escrita protegida e retorna `FALSE` + `ERROR_NOT_SUPPORTED`; resolução de símbolos não é fabricada |
| `dbghelp.dll` | `ImageNtHeader` | Suportado no subconjunto | Copia o DOS header e valida a assinatura PE por leituras protegidas; retorna o endereço do NT header convidado sem acessar seus campos no host |
| `MPR.dll` | `WNetAddConnection2W` / `WNetOpenEnumW` / `WNetEnumResourceW` / `WNetCloseEnum` / `WNetGetResourceInformationW` / `WNetGetResourceParentW` | Não suportado controlado | Todos os seis exports têm classificação explícita `ExportSupport::Stub`, copiam `NETRESOURCEW` e strings aninhadas antes de validar, não criam conexões, credenciais ou handles; saídas `enum_handle`/`system` usam cópias protegidas, buffers de operações rejeitadas antes do consumo não são sondados por snapshot, operações válidas retornam `ERROR_NOT_SUPPORTED` e handles não emitidos retornam `ERROR_INVALID_HANDLE` |
| `POWRPROF.dll` | `PowerGetActiveScheme` / `PowerSetActiveScheme` / `CallNtPowerInformation` | Suportado | `PowerGetActiveScheme` devolve GUID `Balanced` alocado por `LocalAlloc` e liberável por `LocalFree`; saídas e validação de entradas usam cópias protegidas em blocos; `PowerSetActiveScheme` `S_OK`, `CallNtPowerInformation` zera a saída |
...
| Módulo | API | Estado | Comportamento suportado |
|---|---|---|---|
| `WINTRUST.dll` | `WinVerifyTrust` | Suportado no subconjunto | Fotografa ação, `WINTRUST_DATA`, `WINTRUST_BLOB_INFO` e payload `TLTC` por fronteira protegida; valida `WINTRUST_ACTION_GENERIC_VERIFY_V2` + `WTD_CHOICE_BLOB` com cadeia de dois DER, assinatura/validade X.509 e raiz explícita; `state_data` é publicado por escrita protegida e HRESULT não nulo para política ou cadeia inválida |
...
| `CRYPT32.dll` | `CertDuplicateCertificateContext` / `CertFreeCertificateContext` | Suportado no subconjunto | Duplica por fotografia protegida de `CERT_CONTEXT` + DER em armazenamento rastreado, com refcount para cópias emitidas; `CertFreeCertificateContext` libera referências internas; entradas inválidas são rejeitadas |
| `CRYPT32.dll` | `CertGetNameStringW` | Suportado no subconjunto | Fotografa `CERT_CONTEXT` e certificado DER, copia o OID de `CERT_NAME_ATTR_TYPE` e publica UTF-16 com capacidade protegida; extrai `CERT_NAME_SIMPLE_DISPLAY_TYPE`, `CERT_NAME_FRIENDLY_DISPLAY_TYPE`, `CERT_NAME_DNS_TYPE`, `CERT_NAME_EMAIL_TYPE` ou `CERT_NAME_ATTR_TYPE`; sem verificação criptográfica, loja, SAN, Authenticode ou `Cert*` de cadeia |
| `CRYPT32.dll` | `CertNameToStrW` | Suportado no subconjunto | Fotografa `CERT_NAME_BLOB` e DER, converte os tipos simple/OID/X.500 para UTF-16 com separadores, ordem reversa, quoting e limites de buffer no subconjunto publicado, e publica por cópia protegida; sem RDNs multiatributo, loja ou verificação criptográfica |
| `CRYPT32.dll` | `CertGetCertificateContextProperty` | Suportado no subconjunto | Fotografa `CERT_CONTEXT` + DER e publica SHA-1 ou friendly name com `pcbData` protegido; preserva consulta de capacidade e `ERROR_MORE_DATA`; demais propriedades não são implementadas |
| `CRYPT32.dll` | `CertGetEnhancedKeyUsage` / `CertGetIntendedKeyUsage` | Stub controlado | Saídas determinísticas protegidas: 32 bytes zerados para enhanced usage e bytes `0xFF` para a contagem intended; não interpreta extensões EKU nem valida cadeia |
| `CRYPT32.dll` | `CertOpenStore` / `CertCloseStore` | Suportado no subconjunto | Loja em memória e wrappers `CertOpenSystemStoreA/W` com nomes ANSI/UTF-16 copiados por fronteira protegida; provedores não implementados retornam `ERROR_NOT_SUPPORTED`, sem loja Windows ou leitura do trust store Linux |
| `CRYPT32.dll` | `CertEnumCertificatesInStore` / `CertFindCertificateInStore` | Consulta controlada | Handles e contextos são comparados somente dentro do estado emitido pelo runtime; parâmetros SHA-1/subject são copiados por fronteira protegida; loja vazia não fabrica certificados e entradas inválidas falham |
| `CRYPT32.dll` | `CryptQueryObject` | Rejeição controlada | Formatos de consulta ainda não implementados retornam `FALSE` + `ERROR_NOT_SUPPORTED`, zeram as saídas válidas por cópia protegida e nunca fabricam handles ou `CERT_CONTEXT`; Authenticode, arquivos assinados e CMS permanecem fora do contrato |
...
| `CRYPT32.dll` | `CryptMsgGetParam` | Rejeição controlada | Handles nulos ou não emitidos pelo runtime retornam `FALSE` + `ERROR_INVALID_HANDLE` e tamanho zero publicado por cópia protegida; `pcbData` nulo ou inacessível retorna `ERROR_INVALID_PARAMETER`; CMS/Authenticode ainda não são implementados |
...
| `ole32.dll` | `CoCreateGuid` / `CoGetMalloc` / `CoCreateInstance` / `CLSIDFromString` | Suportado no subconjunto | GUIDs, identificadores e ponteiros de saída usam cópias protegidas; `CoCreateInstance` retorna `REGDB_E_CLASSNOTREG` ou `CLASS_E_NOAGGREGATION`, e as conversões determinísticas não fabricam classes COM |
| `ole32.dll` | `DoDragDrop` / `StringFromGUID2` / `CLSIDFromProgID` | Suportado no subconjunto | `DoDragDrop` publica `DROPEFFECT_NONE` e cancela; strings wide são UTF-16 convidadas e saídas usam cópias protegidas |
...
| `USER32.dll` | `GetCursorPos` / `GetCaretPos` / `GetScrollRange` / `GetIconInfo` / `GetIconInfoExW` | Suportado no subconjunto | Estado determinístico de cursor, caret, rolagem e ícones; saídas são publicadas por cópias protegidas |
| `USER32.dll` | `GetUpdateRect` / `GetUserObjectInformationW` / `GetMonitorInfoW` / `GetComboBoxInfo` | Suportado no subconjunto | Retornos e estruturas determinísticas; saídas são publicadas por cópias protegidas |
