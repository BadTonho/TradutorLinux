...
#define TL_VERSION_MSABI __attribute__((ms_abi))
#else
#error "TL_VERSION_MSABI requer GCC ou Clang em x86-64"
#endif
...
TL_VERSION_MSABI int tl_GetFileVersionInfoA(const char* filename, std::uint32_t handle, std::uint32_t len, void* data) noexcept;
TL_VERSION_MSABI int tl_GetFileVersionInfoW(const std::uint16_t* filename, std::uint32_t handle, std::uint32_t len, void* data) noexcept;
