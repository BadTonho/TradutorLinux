...
- Commits desta retomada: `ce66739`, `4f95c65`, `2fa9ebd`, `d9aeb9f`,
  ...
  visual em `d52df26` e hover da navegação em `592a3e5`.
...
- [x] Primeiro shell visual do `7zFM_x64.exe`: o renderer específico de
  ...
  hover com seleção. A árvore lateral também destaca a linha sob o ponteiro sem
  trocar a pasta selecionada.
...
- [x] Controles comuns mínimos: `CreateStatusWindowW` e `CreateToolbarEx`
  ...
  da toolbar por `WM_COMMAND`. `TB_BUTTONSTRUCTSIZE`, `TB_ADDBUTTONS`,
...
- [x] Menu de classe real do `7zFM_x64.exe`: `LoadMenuW` agora resolve o
  ...
  `WM_COMMAND` para itens folha; `Up`/`Down`/`Enter`/`Escape` também operam o
...
- [x] Navegação visual do painel: a lista abre pastas por Enter e por duplo
  ...
  aprovados e 1 skip ambiental).
- [x] Árvore lateral do shell: `Computer`/`Local Disk (Z:)` retornam à raiz
...
- [x] Captura de interação da toolbar: o botão pressionado é redesenhado
...
- [x] Feedback de hover do shell: toolbar e lista acompanham o ponteiro,
  ...
  aprovados e 1 skip ambiental).
- [x] Feedback de hover da navegação lateral: a árvore destaca a linha sob o
  ponteiro, limpa o destaque ao sair e preserva a seleção atual; a regressão
  protege a entrada e a saída do hover. A suíte ficou em 385 testes (384
  aprovados e 1 skip ambiental).
