...
TL_ADVAPI_MSABI int tl_IsTextUnicode(const void* const lpv, const int iSize, int* const lpiResult) noexcept {
    if (lpv == nullptr || iSize < 2) {
        if (lpiResult != nullptr) {
            static_cast<void>(write_guest_value(lpiResult, 0));
        }
        return 0;
    }
    try {
        std::vector<std::uint8_t> bytes(static_cast<std::size_t>(iSize));
        if (runtime::read_guest_memory(lpv, bytes.data(), bytes.size()).status !=
            runtime::GuestMemoryAccessStatus::Success) {
            if (lpiResult != nullptr) {
                static_cast<void>(write_guest_value(lpiResult, 0));
            }
        ...
        if (bytes[0] == 0xFF && bytes[1] == 0xFE) {
            is_unicode = true;
        } else if (bytes[1] == 0x00 && bytes[0] != 0x00) {
            is_unicode = true;
        }
        if (lpiResult != nullptr && !write_guest_value(lpiResult, is_unicode ? 1 : 0)) {
            return 0;
        }
